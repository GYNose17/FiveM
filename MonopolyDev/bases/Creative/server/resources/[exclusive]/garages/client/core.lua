...
-- SPAWNPOSITION
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.SpawnPosition(Select)
    local Slot = "0"
    local Checks = 0
    local Selected = {}
    local Position = nil
...
-- CREATEVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.CreateVehicle(Model, Network, Engine, Health, Customize, Windows, Tyres, Brakes)
...
-- SEARCHBLIP
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.SearchBlip(Coords)
...
-- STARTHOTWIRED
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.StartHotwired()
    Hotwired = true
...
-- STOPHOTWIRED
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.StopHotwired(Vehicle)
    Hotwired = false
...
-- UPDATEHOTWIRED
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.UpdateHotwired(Status)
    Hotwired = Status
end
...
-- REGISTERDECORS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.RegisterDecors(Vehicle)
    SetVehicleHasBeenOwnedByPlayer(Vehicle, true)
    SetVehicleNeedsToBeHotwired(Vehicle, false)
    DecorSetInt(Vehicle, "Player_Vehicle", -1)
    SetVehRadioStation(Vehicle, "OFF")
        ...
        if not IsPedInAnyVehicle(Ped) then
            ...
            for Number, v in pairs(GaragesCoords) do
                ...
                if Distance <= 5.0 then
                    ...
                    if Distance <= 1.25 and IsControlJustPressed(1, 38) and not exports["hud"]:Reposed() and not exports["hud"]:Wanted() and not LocalPlayer["state"]["usingPhone"] and not LocalPlayer["state"]["Target"] then
...
RegisterNUICallback("Spawn", function(Data, Callback)
    if Data["Name"] then
        vSERVER.Spawn(Data["Name"], Data["Number"])
    end
