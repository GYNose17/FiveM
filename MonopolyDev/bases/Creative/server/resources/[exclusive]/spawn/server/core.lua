...
-----------------------------------------------------------------------------------------------------------------------------------------
Baixada = {}
Tunnel.bindInterface("spawn", Baixada)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local Global = {}
...
-- CHARACTERCHOSEN
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.CharacterChosen(Passport)
	local source = source
	local License = vRP.Identities(source)
...
-- NEWCHARACTER
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.NewCharacter(name, lastname, sex)
	local source = source
	if not Global[source] then
		...
		if parseInt(Account["Characters"]) <= parseInt(Consult[1]["Amount"]) then
		...
		if Last[1] then
			TriggerEvent("vRP:BucketServer", source, "Exit")
			vRP.CharacterChosen(source, Last[1]["id"], sex)
		end
