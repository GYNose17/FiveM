...
-----------------------------------------------------------------------------------------------------------------------------------------
Baixada = {}
Tunnel.bindInterface("slotmachine", Baixada)
	...
	if LocalPlayer["state"]["Cassino"] then
		...
		if Selected ~= nil then
			if vSERVER.Check(Selected) then
				...
				local Chairs = GetWorldPositionOfEntityBone(Entity[1],
					GetEntityBoneIndexByName(Entity[1], "Chair_Base_01"))
				...
				if LoadModel(Machines[Selected]["prop"]) then
					Spin01 = CreateObject(Machines[Selected]["prop"], Offset01["x"], Offset01["y"], Offset01["z"], false,
						false)
					Spin02 = CreateObject(Machines[Selected]["prop"], Offset02["x"], Offset02["y"], Offset02["z"], false,
						false)
					Spin03 = CreateObject(Machines[Selected]["prop"], Offset03["x"], Offset03["y"], Offset03["z"], false,
						false)
						...
						while Spin01 ~= nil do
							if IsDisabledControlJustPressed(0, 47) then
								if not Spinning then
									...
									SetEntityCoords(Ped, Leave["x"], Leave["y"], Leave["z"] + 1.0, false, false, false,
										false)
							...
							if IsDisabledControlJustPressed(0, 38) then
								if not Spinning then
									if vSERVER.Payment(Selected) then
										Spinning = true
										vRP.PlayAnim(true, { "anim_casino_a@amb@casino@games@slots@female",
											"press_spin_a" }, false)
				...
				TriggerEvent("Notify", "vermelho", "Máquina ocupada.", "Atenção", 5000)
			end
		end
	end
end)
...
-- MACHINESLOTS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.MachineSlots(Result)
	if Spinning then
		for i = 1, 300, 1 do
			...
			if i < 180 then
				SetEntityRotation(Spin01, rotSpin01["x"] + math.random(40, 100) / 10, rotSpin01["y"], rotSpin01["z"], 1,
					true)
			elseif i == 180 then
			...
			if i < 240 then
				SetEntityRotation(Spin02, rotSpin02["x"] + math.random(40, 100) / 10, rotSpin02["y"], rotSpin02["z"], 1,
					true)
			elseif i == 240 then
			...
			if i < 300 then
				SetEntityRotation(Spin03, rotSpin03["x"] + math.random(40, 100) / 10, rotSpin03["y"], rotSpin03["z"], 1,
					true)
			elseif i == 300 then
...
-- UPDATEMACHINES
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.UpdateMachines(Table)
	Machines = Table
end
