...
-- PAYMENT
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Payment(Table)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and Machines[Table] then
		if vRP.TakeItem(Passport, "chips", Machines[Table]["bet"], true) then
			return true
		end
	end
...
-- STARTSLOTS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.StartSlots(Table)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and Machines[Table] then
		local Result = {
...
-- WINNER
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Winner(Table, Result)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] and Machines[Table] then
		...
		if Machines[Table]["win"] then
			if Machines[Table]["win"]["a"] == Result["a"] and Machines[Table]["win"]["b"] == Result["b"] and Machines[Table]["win"]["c"] == Result["c"] then
				...
				if Total > 0 then
					vRP.GenerateItem(Passport, "chips", Total, true)
				end
			end
