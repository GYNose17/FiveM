...
-- PARACHUTE
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Parachute()
	GiveWeaponToPed(PlayerPedId(), "GADGET_PARACHUTE", 1, false, true)
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- FISHING
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Fishing()
	local Ped = PlayerPedId()
	local Coords = GetEntityCoords(Ped)
	local Fishings = {
		vec3(-1227.49,4389.84,5.12)
	}

	for i = 1, #Fishings do
		if #(Coords - Fishings[i]) <= 150 and IsEntityInWater(Ped) then
			return true
		end
	end
...
-- RETURNWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.ReturnWeapon()
	return Weapon ~= "" and Weapon or false
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECKWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.CheckWeapon(Hash)
...
-- CHECKATTACHS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.CheckAttachs(Name,Hash)
	return Attachs[Name] and Attachs[Name][Hash]
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- EXISTATTACHS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.ExistAttachs(Weapon)
	local Return = false
...
-- PUTATTACHS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Attachs(Name,Hash)
	GiveWeaponComponentToPed(PlayerPedId(),Hash,Attachs[Name][Hash])
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- PUTWEAPONHANDS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.TakeWeapon(Name, Ammo, Components, Type)
	if not TakeWeapon then
		...
		if itemAmmo(Name) then
			TriggerEvent("hud:Weapon", true, Name, itemName(Name))
		end

		if not MumbleIsConnected() or vSERVER.CheckExistWeapons(Name) or LocalPlayer["state"]["Safezone"] then
...
-- INFOWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.InfoWeapon(Type)
	local Ammo = 0
	local Hash = nil
	local Status = false
...
-- RELOADING
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Reloading(Hash,Ammo)
...
CreateThread(function()
	while true do
		...
		if Actived and Weapon ~= "" then
			...
			if Ammo <= 0 or (Weapon == "WEAPON_PETROLCAN" and Ammo <= 135 and IsPedShooting(Ped)) or (Weapon == "WEAPON_FIREEXTINGUISHER" and Ammo <= 135 and IsPedShooting(Ped)) or IsPedSwimming(Ped) then
				if Types ~= "" then
...
-- CHECKFOUNTAIN
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.checkFountain()
	local Ped = PlayerPedId()
	local Coords = GetEntityCoords(Ped)
...
-- TYREHEALTH
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.tyreHealth(Network,Tyre)
	if NetworkDoesNetworkIdExist(Network) then
		local Vehicle = NetToEnt(Network)
		if DoesEntityExist(Vehicle) then
...
-- CHECKARMS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.CheckArms()
	return exports["modules"]:Arms()
end

-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECKMODS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.CheckMods(Vehicle, Mod)
...
-- CHECKCAR
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.CheckCar(Vehicle)
...
-- ACTIVEMODS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.ActiveMods(Vehicle, Plate, Mod, Number)
	if NetworkDoesNetworkIdExist(Vehicle) then
		local Vehicle = NetToEnt(Vehicle)
		if DoesEntityExist(Vehicle) then
			if GetVehicleNumberPlateText(Vehicle) == Plate then
...
-- RESTAURANT
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Restaurant(Name)
	local Return = false
	local Ped = PlayerPedId()
	local Coords = GetEntityCoords(Ped)

	if Name == "Burgershot" and Burgershot:isPointInside(Coords) then
		Return = true
	end
