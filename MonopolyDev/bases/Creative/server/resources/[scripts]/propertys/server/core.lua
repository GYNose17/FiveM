...
-----------------------------------------------------------------------------------------------------------------------------------------
Baixada = {}
Tunnel.bindInterface("propertys",Baixada)
...
-- ROBBERY
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Robbery(Name,Number)
	local source = source
...
-- POLICE
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Police(Coords)
	local source = source
...
-- PAYBERRY
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Paybbery(Name,Number)
	local source = source
	local Passport = vRP.Passport(source)
...
-- PROPERTYS
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Propertys(Name)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
-- TOGGLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Toggle(Name,Mode)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
		if Mode == "Exit" then
		...
		if not Consult[1] then
			...
			if vRP.Request(source,"Propriedades","Deseja comprar a propriedade?") then
				if vRP.PaymentFull(Passport,Informations[Interior]["Price"]) then
					...
					if NewBankTaxs then
						exports["bank"]:AddTaxs(Passport,"Propriedades",Informations[Interior]["Price"] * 0.1,"Compra de propriedade.")
					end
	...
	if Passport and not Actived[Passport] then
		...
		if Consult[1] and Consult[1]["Passport"] == Passport then
			...
			if vRP.Request(source,"Propriedades","Vender por <b>$"..parseFormat(Price).."</b>?") then
				...
				vRP.RemServerData("Vault:"..Name,true)
				vRP.RemServerData("Fridge:"..Name,true)
...
-- PERMISSION
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Permission(Name)
	local source = source
	local Passport = vRP.Passport(source)
	local Consult = vRP.Query("propertys/Exist",{ Name = Name })
	if Passport and Consult[1] and (vRP.InventoryFull(Passport,"propertys-"..Consult[1]["Serial"]) or Consult[1]["Passport"] == Passport) then
...
-- REQUEST
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Request(Name,Mode)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
		local Chest = {}
...
-- STORE
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Store(Item,Slot,Amount,Target,Name,Mode)
	local source = source
...
-- TAKE
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Take(Slot,Amount,Target,Name,Mode)
	local source = source
	local Amount = parseInt(Amount)
	local Passport = vRP.Passport(source)
	if Passport then
...
-- UPDATE
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Update(Slot,Target,Amount,Name,Mode)
	local source = source
	local Amount = parseInt(Amount)
	local Passport = vRP.Passport(source)
	if Passport then
...
CreateThread(function()
	...
	for _,v in pairs(Consult) do
		...
		if (v["Tax"] + 604800) <= os.time() then
			vRP.RemServerData("Vault:"..Name)
			vRP.RemServerData("Fridge:"..Name)
			vRP.Query("propertys/Sell",{ Name = Name })
		else
