...
Tunnel.bindInterface("luckywheel", Baixada)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local Bonus = {}
...
local ItemQuantity = 15000
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECK
-----------------------------------------------------------------------------------------------------------------------------------------
function Baixada.Check()
	local source = source
	...
	if Passport then
		if os.time() > Active then
			...
			if vRP.TakeItem(Passport, "chips", ItemQuantity, true) then
				Active = os.time() + 20
				return true
			else
				TriggerClientEvent("Notify", source, "vermelho",
					"Você não tem <b>" .. parseFormat(ItemQuantity) .. "x " .. itemName("chips") .. "</b>.", "Aviso",
					5000)
			end
		else
		...
		elseif Payments[Passport] == 19 then
			...
			if Vehicle[1] then
				...
				vRP.Query("vehicles/rentalVehicles",
					{ Passport = Passport, vehicle = vehName, plate = vRP.GeneratePlate(), work = "false" })
			end
		elseif Payments[Passport] == 20 then
			vRP.GenerateItem(Passport, "chips", 25000, true)
		end
