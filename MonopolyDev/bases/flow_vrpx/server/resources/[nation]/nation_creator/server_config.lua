...
if multiCharacter then
    ...
    vRP._prepare("nation_creator/update_user_first_spawn","UPDATE vrp_user_identities SET sobrenome = @sobrenome, nome = @nome, idade = @idade WHERE user_id = @user_id")
...
function func.saveChar(name, lastName, age, char, id)
	...
	--print("funcao 2", user_id)
    if char then
        vRP.setUData(user_id, "nation_char", json.encode(char,{indent=false}))
    end
    if name and lastName and age then
        vRP.execute("nation_creator/update_user_first_spawn",{ user_id = user_id, sobrenome = lastName, nome = name, idade = age })
    end
...
function playerSpawn(user_id, source, first_spawn)
	--print("funcao 4", user_id)
    if first_spawn then
        --vRP.addUserGroup(1,"Fundador")
        TriggerEvent("cashop:resetdimensao")
        Wait(1000)
...
function processSpawnController(source,char,user_id)
	...
	if char then
        ...
        if data and data ~= "" then 
            ...
            TriggerEvent("cashop:setardimensao")
        end
	end
end


RegisterServerEvent("cashop:setardimensao")
AddEventHandler("cashop:setardimensao",function(id)
    local source = source
    local user_id = id or vRP.getUserId(source)
    if user_id then
...
RegisterServerEvent("cashop:resetdimensao")
AddEventHandler("cashop:resetdimensao",function()
	local source = source
    local user_id = id or vRP.getUserId(source)
    if user_id then
...
function func.updateLogin()
    local source = source
    local user_id = vRP.getUserId(source)
	--print("funcao 10", user_id)
	exports.bm_module:getAccount(source)
	TriggerClientEvent("llimao_hudcommand",source,true)
    if user_id then
        userlogin[user_id] = true
...
function func.getCharsInfo()
    ...
    for k,v in ipairs(data) do
        ...
        info.chars[k] = {
            name = teste[1].nome.." "..teste[1].sobrenome, age = teste[1].idade.." anos", bank = "$ "..format(bank), clothes = clothes,
...
function getUserClothes(user_id)
	...
	--print("funcao 12", user_id)
	local source = vRP.getUserSource(user_id)
	TriggerClientEvent("llimao_hudcommand",source,true)
	print('mostrando hud para o user id: '..user_id..' com o source:', source)
    if data and data ~= "" then
...
RegisterCommand('plastica', function(source, args)
    local userSource = source
    local hasPlastica = false
    if vRP.getInventoryItemAmount(userSource, "plastica") > 0 then
        hasPlastica = true
    end
    if hasPlastica and checkItensD(userSource) then
        if args[1] then
            local id = tonumber(args[1])
            if id then
                local src = vRP.getUserSource(id)
                if src and vRP.request(userSource, "Deseja resetar o ID "..id.."?", 30) then
                    fclient._startCreator(src)
                    vRP.tryGetInventoryItem(userSource, "plastica", 1) 
                end
            end
        else
            if vRP.request(userSource, "Deseja resetar seu personagem?", 30) then
                fclient._startCreator(userSource)
                vRP.tryGetInventoryItem(userSource, "plastica", 1) 
            end
        end
    end
...
function checkItensD(source)
    local user_id = vRP.getUserId(source)
    if user_id then
        local mensagem = ""
        local status = true
        for k, v in pairs(itemplastica) do
...
RegisterCommand('spawn',function(source) -- COMANDO DE ADMIN PARA SIMULAR O SPAWN
    if func.checkPermission({"devepoler", "developer.permissao", "Admin"}, source) or not vRP.getUserId(source) then
        if multiCharacter then
