...
AddEventHandler('__cfx_internal:commandFallback', function(command)
    ...
    TriggerEvent('chatMessage', source, nome, '/' .. command)

   -- if not WasEventCanceled() then
      --  TriggerClientEvent('chatMessage', -1, nome, { 255, 255, 255 }, '/' .. command) 
    --end
...
local function refreshCommands(player)
    if GetRegisteredCommands then
        ...
        for _, command in ipairs(registeredCommands) do
            if IsPlayerAceAllowed(player, ('command.%s'):format(command.nome)) then
...
AddEventHandler('onServerResourceStart', function(resnome)
    Wait(500)
    ...
    RegisterCommand('ilegal', function(source, args, rawCommand)
        ...
        if user_id ~= nil then
            for k, v in pairs(vRP.getUsers()) do
                if not vRP.terPemissao(k, "policia.permissao") then
    ...
    RegisterCommand('admin', function(source, args, rawCommand)
        ...
        local identity = vRP.getUserIdentity(user_id)
        fal = identity.nome.. " " .. identity.sobrenome
        if vRP.terPemissao(user_id, "owner.permissao") or vRP.terPemissao(user_id, "admin.permissao") or vRP.terPemissao(user_id, "moderador.permissao") or vRP.terPemissao(user_id, "wl.permissao") then
        TriggerClientEvent('chat:addMessage', -1, {
