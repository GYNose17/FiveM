...
-- BANS
vRP.prepare("flow/getAllBans","SELECT * FROM flow_bans WHERE time < UNIX_TIMESTAMP() AND time > 0")
vRP.prepare("flow/getUserBanned","SELECT * FROM flow_bans WHERE user_id = @user_id")
vRP.prepare("flow/insertBanned","INSERT IGNORE INTO flow_bans(user_id,motivo,desbanimento,banimento,time, hwid) VALUES(@user_id,@motivo,@desbanimento,@banimento,@time, @hwid)")
vRP.prepare("flow/addToken","INSERT IGNORE INTO flow_bans_hwid(id,token) VALUES(@user_id,@token)")
vRP.prepare("flow/select_hwid", "SELECT * FROM flow_bans_hwid mhb LEFT JOIN flow_bans mh ON (mh.user_id = mhb.id) WHERE mhb.token = @token AND hwid = '1'")
vRP.prepare("flow/removeBanned", "DELETE FROM flow_bans WHERE user_id = @user_id")
vRP.prepare("flow/createBanDB",[[ CREATE TABLE IF NOT EXISTS `flow_bans` ( `user_id` int(11) NOT NULL, `motivo` text NOT NULL, `banimento` tinytext NOT NULL, `desbanimento` tinytext NOT NULL, `time` int(11) NOT NULL, `hwid` int(11) NOT NULL, PRIMARY KEY (`user_id`) USING BTREE ) ENGINE=InnoDB DEFAULT CHARSET=latin1; ]])
vRP.prepare("flow/createBanDBHWID",[[ CREATE TABLE IF NOT EXISTS `flow_bans_hwid` ( `token` varchar(120) NOT NULL, `id` int(11) NOT NULL, PRIMARY KEY (`token`), KEY `id` (`id`) ) ENGINE=InnoDB DEFAULT CHARSET=latin1; ]])
