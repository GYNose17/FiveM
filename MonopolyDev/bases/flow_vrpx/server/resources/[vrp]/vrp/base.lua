...
local geral = {
    ...
    discord = "https://discord.gg/cEwAsnrDe7", -- Discord do Servidor (Colocar https://)

    color = 6356736, -- Cor da Lateral do WeebHook
    footer = "© Vrpx", -- RODAPE do WeebHook
...
vRP.prepare("flow/insertBanned/base","INSERT IGNORE INTO flow_bans(user_id,motivo,desbanimento,banimento,time, hwid) VALUES(@user_id,@motivo,@desbanimento,@banimento,@time, @hwid)")
function vRP.setBanned(user_id,status)
	if user_id then
		vRP.execute("flow/insertBanned/base", { user_id = user_id, motivo = "Banido pelo ANTI HACK", banimento = os.date("%d/%m/%Y as %H:%M"), desbanimento = "Nunca", time = 0, hwid = 0 })
	end
end
...
-- HWID SYSTEM
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("getAllTokens", "SELECT token,id FROM flow_bans_hwid")
vRP.prepare("flow_bans/getUserBans", "SELECT user_id,banimento,desbanimento,motivo FROM flow_bans WHERE user_id = @user_id")
vRP.prepare("flow_bans/addToken","INSERT IGNORE INTO flow_bans_hwid(id,token) VALUES(@user_id,@token)")
	...
	for i = 1, #tokens do
		if self.list[tokens[i]] then
			if self.list[tokens[i]] ~= user_id then
				local query = vRP.query("flow_bans/getUserBans", { user_id = self.list[tokens[i]] })
				if query and #query > 0 then
					vRP.execute("flow/insertBanned", { user_id = user_id, motivo = "Banido HWID! ID ANTIGO: "..self.list[tokens[i]], banimento = os.date("%d/%m/%Y as %H:%M"), desbanimento = "Nunca", time = 0, hwid = 0 })
			...
			vRP.execute("flow_bans/addToken", { user_id = user_id, token = tokens[i] })
			self.list[tokens[i]] = user_id
		end
	end

	local query = vRP.query("flow_bans/getUserBans", { user_id = user_id })
	if query and #query > 0 then
...
AddEventHandler("playerConnecting", function(name, setKickReason, deferrals)
	if not GeneratingCache then deferrals.update(" Ops, houve um problema. tente novamente.") return end
