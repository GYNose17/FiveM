	...
	window.addEventListener("message",function(event){
		if (event.data.quantidade != undefined || event.data.item != undefined || event.data.mode != undefined ) { 
			...
			if (event.data.mode == 'sucesso') {
				html = "<div id='content'> <div id='imagem' style=\"background-image: url('http://104.234.65.51:/ultimate/inventario/"+event.data.item+".png'); \"> </div> <div id='nome'>"+event.data.nome+"</div> <div id='quantidade'><sucesso>+ </sucesso>"+event.data.quantidade+"</div> <div id='peso'>("+event.data.kgTotal.toFixed(2)+")</div> </div>"
				
			}else if (event.data.mode == 'negado') { 
				html = "<div id='content'> <div id='imagem' style=\"background-image: url('http://104.234.65.51:/ultimate/inventario/"+event.data.item+".png'); \"></div> <div id='nome'>"+event.data.nome+"</div> <div id='quantidade'><negado>- </negado>"+event.data.quantidade+"</div> <div id='peso'>("+event.data.kgTotal.toFixed(2)+")</div> </div>"
			}
