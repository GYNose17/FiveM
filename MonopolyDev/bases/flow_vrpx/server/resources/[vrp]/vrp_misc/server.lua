...
src = {}
Tunnel.bindInterface("flow_bans",src)
Proxy.addInterface("flow_bans",src)

vCLIENT = Tunnel.getInterface("flow_bans")
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
src.setBanned = function(source, target_id, motivo, tempo, hwid)
    if autenticado then
        ...
        local rows = vRP.query("flow/getUserBanned", { user_id = target_id })
        ...
        if user_id then
            ...
            vRP.execute("flow/insertBanned", { user_id = target_id, motivo = motivo, banimento = dataBan, desbanimento = dataUnban, time = timeBan, hwid = hwid })
...
src.setUnBanned = function(source, target_id, motivo)
    if autenticado then
        ...
        if user_id then
            ...
            totalBanidos = totalBanidos - 1
            vRP.execute("flow/removeBanned", { user_id = target_id })
...
Citizen.CreateThread(function()
    while true do
        if autenticado then
            local rows = vRP.query("flow/getAllBans", {})
            if #rows > 0 then
                for k,v in pairs(rows) do
                    if v.time > 0 and os.time() > v.time then
                        vRP.execute("flow/removeBanned", { user_id = v.user_id })
