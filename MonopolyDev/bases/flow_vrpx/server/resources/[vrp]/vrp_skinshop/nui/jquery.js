    ...
    $('.itemroupa').click(function(){
        ...
        if (item){
            ...
            $.post('http://vrp_skinshop/trocarClasse', JSON.stringify({ part: item }))
    ...
    $(document).ready(function(){    
        window.addEventListener("message",function(event){
            ...
            if (item.changeCategory) {
                ...
                $('.item-clothe').click(function(){
                    ...
                    $.post('http://vrp_skinshop/changeCustom', JSON.stringify({ type: dataPart, id: roupa }));
                })
            }
            if (item.value){
        ...
        document.onkeyup = function(data){
            if (data.which == 27){
                $.post("http://vrp_skinshop/roupasClose",JSON.stringify({}),function(datab){});
                $('.zoom-escolharoupa').fadeOut()
            }
        };
    })
    ...
    $('.aceitarroupa').click(function(){
        $.post('http://vrp_skinshop/payament', JSON.stringify({ price: valorCarrinho }));
        $('.confirmacaoroupa').fadeOut()
    })

    $(".cor-roupa").click(function() {
        if ($(this).hasClass('maisroupa')){
            $.post('http://vrp_skinshop/changeColor', JSON.stringify({ type: dataPart, id: roupa, action: "mais" }));
        }
        if ($(this).hasClass('menosroupa')){
            $.post('http://vrp_skinshop/changeColor', JSON.stringify({ type: dataPart, id: roupa, action: "menos" }));
        }
    })

    function onRangeChange(tipo,value) { 
        $.post('https://vrp_skinshop/updateRotate', JSON.stringify({
            tipo: tipo,
            valor: value
        }));
    }
