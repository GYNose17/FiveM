...
dPN = {}
Tunnel.bindInterface("flow_tatoo",dPN)
Proxy.addInterface("flow_tatoo",dPN)
dPNclient = Tunnel.getInterface("flow_tatoo")
...
RegisterServerEvent('flow_tatoo:setPedClient')
AddEventHandler('flow_tatoo:setPedClient', function()
    local source = source
    local user_id = vRP.getUserId(source)
    local value = vRP.getUData(parseInt(user_id),"vRP:tattoos")
    local custom = json.decode(value) or {}
...
RegisterServerEvent('flow_tatoo:setPedServer')
AddEventHandler('flow_tatoo:setPedServer', function(source)
    if source then
        local user_id = vRP.getUserId(source)
        if user_id then
            local value = vRP.getUData(user_id,    "vRP:tattoos   ")
