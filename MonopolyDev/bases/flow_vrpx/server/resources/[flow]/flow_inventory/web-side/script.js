...
$(document).ready(function () {
    ...
    document.onkeyup = data => {
        const key = data.key;
        if (key === "Escape") {
            $.post("http://flow_inventory/invClose");
        } else if (key === "Shift") {
            shiftPressed = false;
        }
    };
    ...
    $('.close').click(function () {
        $.post("http://flow_inventory/invClose", JSON.stringify({}));
    });
});
...
const updateMochila = () => {
    $.post("http://flow_inventory/requestMochila", JSON.stringify({}), (data) => {
        ...
        for (let x = 1; x <= mySlots; x++) {
            ...
            if (data.inventario[slot] !== undefined) {
                ...
                const item = `
                <div class="item populated" style="background-image: url('http://127.0.0.1:/ultimate/inventario/${v.key}.png'); background-position: center; background-size: 55% !important; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-amount="${v.amount}" data-slot="${slot}">
                    <div class="top_item">
        ...
        for (let x = 1; x <= inSlots; x++) {
            ...
            if (nameList2[x - 1] !== undefined) {
                ...
                const item = `
                <div class="item populated" style="background-image: url('http://127.0.0.1:/ultimate/inventario/${v.key}.png'); background-position: center; background-size: 55% !important; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-id="${v.id}" data-grid="${v.grid}" data-amount="${v.amount}" data-peso="${v.peso}" data-slot="${slot}">
                    <div class="top_item">
                        <div class="item_amount">x${formatarNumero(v.amount)}</div>
                        <div class="item_weight">
                            <img src="images/weight.svg" alt="">
...
const updateDrag = () => {
    ...
    $('.empty').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (origin === "invLeft" && tInv === "invLeft") {
                $.post("http://flow_inventory/updateSlot", JSON.stringify({
                    item: itemData.key,
                    slot: itemData.slot,
                ...
                $.post("http://flow_inventory/pickupItem", JSON.stringify({
                    id: id,
                    grid: grid,
                    target: target,
                    amount: parseInt(amount)
                ...
                $.post("http://flow_inventory/dropItem", JSON.stringify({
                    item: itemData.key,
                    slot: itemData.slot,
                    amount: parseInt(amount)
                }));
    ...
    $('.use').droppable({
        ...
        drop: function (event, ui) {
            ...
            $.post("http://flow_inventory/useItem", JSON.stringify({
                item: itemData.key,
                slot: itemData.slot,
    ...
    $('.send').droppable({
        ...
        drop: function (event, ui) {
            ...
            $.post("http://flow_inventory/sendItem", JSON.stringify({
                item: itemData.key,
                slot: itemData.slot,
                amount: amount
            }));
...
const requestVehicles = () => {
    $.post("http://flow_inventory/requestVehicle", JSON.stringify({}), (data) => {
        ...
        for (let x = 1; x <= inSlots; x++) {
            ...
            if (data.inventario2[slot] !== undefined) {
                ...
                const item = `
                <div class="item populated" style="background-image: url('http://127.0.0.1:/ultimate/inventario/${v.key}.png'); background-size: 55% !important; background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-amount="${v.amount}" data-slot="${slot}">
                    <div class="top_item">
                        <div class="item_amount">x${formatarNumero(v.amount)}</div>
                        <div class="item_weight">
                            <img src="images/weight.svg" alt="">
        ...
        drop: function (event, ui) {
            ...
            if (tInv === "invLeft") {
                if (origin === "invLeft") {
                    ...
                    $.post("http://flow_inventory/takeVehicle", JSON.stringify({
                        item: itemData.key,
                        slot: itemData.slot,
                        target: target,
                        amount: amount
                ...
                if (origin === "invRight") {
                    $.post("http://flow_inventory/updateVehicleSlots", JSON.stringify({
                        item: itemData.key,
                        slot: itemData.slot,
                        target: target,
                        amount: amount
                    ...
                    $.post("http://flow_inventory/storeVehicle", JSON.stringify({
                        item: itemData.key,
                        slot: target,
                        amount: amount
                    }));
...
const requestOrgChest = () => {
    $.post("http://flow_inventory/requestOrgChest", JSON.stringify({}), (data) => {
        // let porcentage = data.peso * 100 / data.maxpeso;
        // weight_bar.set(porcentage)
        $('.weight_info').html(`
            ${(data.peso).toFixed(2)}/${(data.maxpeso).toFixed(2)}
        ...
        drop: function (event, ui) {
            ...
            if (tInv === "invLeft") {
                if (origin === "invLeft") {
                    ...
                    $.post("http://flow_inventory/takeOrgChest", JSON.stringify({
                        item: itemData.key,
                        slot: itemData.slot,
                        target: target,
                        amount: amount
                ...
                if (origin === "invRight") {
                    $.post("http://flow_inventory/updateOrgSlots", JSON.stringify({
                        item: itemData.key,
                        slot: itemData.slot,
                        target: target,
                        amount: amount
                    ...
                    $.post("http://flow_inventory/storeOrgChest", JSON.stringify({
                        item: itemData.key,
                        slot: target,
                        amount: amount
                    }));
...
const requestHouseChest = () => {
    $.post("http://flow_inventory/requestHouseChest", JSON.stringify({}), (data) => {
        // let porcentage = data.peso * 100 / data.maxpeso;
        // weight_bar.set(porcentage)
        $('.weight_info').html(`
            ${(data.peso).toFixed(2)}/${(data.maxpeso).toFixed(2)}
        ...
        drop: function (event, ui) {
            ...
            if (tInv === "invLeft") {
                if (origin === "invLeft") {
                    ...
                    $.post("http://flow_inventory/takeHouseChest", JSON.stringify({
                        item: itemData.key,
                        slot: itemData.slot,
                        target: target,
                        amount: amount
                ...
                if (origin === "invRight") {
                    $.post("http://flow_inventory/updateChestSlots", JSON.stringify({
                        item: itemData.key,
                        slot: itemData.slot,
                        target: target,
                        amount: amount
                    ...
                    $.post("http://flow_inventory/storeHouseChest", JSON.stringify({
                        item: itemData.key,
                        slot: target,
                        amount: amount
                    }));
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ */
const requestStore = () => {
    $.post("http://flow_inventory/requestStore", JSON.stringify({}), (data) => {
        // let porcentage = data.peso * 100 / data.maxpeso;
        // weight_bar.set(porcentage)
        $('.weight_info').html(`
            ${(data.peso).toFixed(2)}/${(data.maxpeso).toFixed(2)}
        ...
        drop: function (event, ui) {
            ...
            if (tInv === "invLeft") {
                if (origin === "invLeft") {
                    ...
                    $.post("http://flow_inventory/buyStore", JSON.stringify({
                        item: itemData.key,
                        target: target,
                        amount: buyAmount
                    }));
                ...
                if (origin === "invLeft") {
                    $.post("http://flow_inventory/sellStore", JSON.stringify({
                        item: itemData.key,
                        amount: parseInt(amount)
                    }));
                }
...
const requestRevistar = () => {
    $.post("http://flow_inventory/requestRevistar", JSON.stringify({}), (data) => {
        ...
        for (let x = 1; x <= inSlots; x++) {
            ...
            if (data.inventario2[slot] !== undefined) {
                ...
                const item = `
                <div class="item populated" style="background-image: url('http://127.0.0.1:/ultimate/inventario/${v.key}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-amount="${v.amount}" data-slot="${slot}">
                    <div class="top_item">
                        <div class="item_amount">x${formatarNumero(v.amount)}</div>
                        <div class="item_weight">
                            <img src="images/weight.svg" alt="">
        ...
        drop: function (event, ui) {
            ...
            if (tInv === "invLeft") {
                if (origin === "invLeft") {
                    ...
                    $.post("http://flow_inventory/retirarItemRevistar", JSON.stringify({
                        item: itemData.key,
                        target: target,
                        slot: itemData.slot,
                        amount: amount
               ...
               if (origin === "invLeft") {
                    $.post("http://flow_inventory/colocarItemRevistar", JSON.stringify({
                        item: itemData.key,
                        slot: target,
                        amount: amount
                    }));
