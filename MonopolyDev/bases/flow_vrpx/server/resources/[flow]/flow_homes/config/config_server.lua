...
src = {}
Tunnel.bindInterface("flow_homes",src)
Proxy.addInterface("flow_homes",src)

vCLIENT = Tunnel.getInterface("flow_homes")
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- MAIN
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
config = {} -- Não mexer
...
-- COMANDOS
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand('criarcasas', function(source,args) -- Comando criarcasa
            ...
            if idProp ~= nil and tonumber(idProp) then
                if src.checkIdHouse(idProp) then
                    if vRP.hasGroup(user_id, "valegaragem") then
                        local owner = vRP.query("flow/selectProprietario", { proprietario = user_id })
                        if #owner > 0 then
...
RegisterCommand('homes', function(source,args) -- Comando gerais das casas
    local user_id = vRP.getUserId(source)
    if user_id then
        ...
        if args[1] == "iptu" and tonumber(args[2]) then
            ...
            if owner.iptu <= os.time() + config.iptuVencimento*24*60*60 then
                ...
                if vRP.tryFullPayment(user_id, price) then
                    ...
                    vRP.execute("flow/updateIptu", { iptu = (os.time() + config.sellHouseIptu*24*60*60) , id = owner.id })
        ...
        if args[1] == "ver" and tonumber(args[2]) then
            ...
            local owner = vRP.query("flow/selectProprietario", { proprietario = tonumber(args[2]) })
            if #owner == 0 then
        ...
        if args[1] == "vender" and tonumber(args[2]) then
            ...
            vRP.execute("flow/updateOwner", { proprietario = nuser_id, id = owner.id })
        ...
        TriggerClientEvent('chatMessage', source, '^1[COMANDOS]: \n^2/homes list ^0 - mostra lista dos ids de suas propriedades\n^2/homes disp^0 - mostra a lista de propriedades disponivel para comprar\n^2/homes iptu [houseID]^0 - para consultar/pagar o iptu\n^2/homes moradores [houseID]^0 - mostra a lista de moradores em sua propriedade\n^2/homes vender [houseID]^0 - vende a sua propriedade\n^2/homes localizar [houseID]^0 - localiza a propriedade no mapa (policiais)\n^2/homes ver [userID] ^0 - ve a lista de propriedades do jogador (policiais)   ')
    end
end)
...
src.comprarPropriedade = function(id, tipo)
    ...
    local user_id = vRP.getUserId(source)
    if user_id then
        local owner = vRP.query("flow/ownerPropriedade", { houseID = id })
        ...
        if tipo == "apartamento" then
            ...
            local owner2 = vRP.query("flow/allAPOwner", { houseID = id, proprietario = user_id })
            if #owner2 > 0 then
...
src.myVehicles = function(user_id) -- Captura seus Veiculos
    ...
    local myVehs = vRP.query("flow_homes/myVehicles", {user_id = user_id})
...
src.isMyVehicle = function(user_id, veiculo) -- Verifica se o veiculo realmente existe na garagem dele
    local source = vRP.getUserSource(user_id)
    local myVehs = vRP.query("flow_homes/isMyVehicle", {user_id = user_id, veiculo = veiculo})
...
src.openBau = function(user_id, id, houseId)
    local source = vRP.getUserSource(user_id)
    TriggerClientEvent("fl0w:myHouseChest", source, id, houseId, propriedades[houseId].minBau)
end
...
Citizen.CreateThread(function()
    while true do
        local query = vRP.query("flow/getHousesIPTU", {})
        if #query > 0 then
            for k,v in pairs(query) do
                ...
                vRP.execute("flow/deleteUserProp", { id = v.id })		
...
-- QUERYS
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("flow/allPropriedades","SELECT * FROM flow_homes")
vRP.prepare("flow/allMoradores","SELECT * FROM flow_users_homes")
vRP.prepare("flow/criarPropriedade","INSERT INTO flow_homes(interior,tipo,price,chaves,minBau,coords,permissao) VALUES(@interior, @tipo,@price,@chaves,@minBau,@coords,@permissao);")
vRP.prepare("flow/selecionarPropriedade","SELECT * FROM flow_homes WHERE id = @houseID")
vRP.prepare("flow/deletarPropriedade","DELETE FROM flow_homes WHERE id = @houseID")
vRP.prepare("flow/comprarPropriedade","INSERT INTO flow_users_homes (tipo,houseID,proprietario,interior,iptu) VALUES(@tipo,@houseID,@proprietario,@interior,@iptu)")
vRP.prepare("flow/ownerPropriedade","SELECT * FROM flow_users_homes WHERE houseID = @houseID")
vRP.prepare("flow/allHomeOwner","SELECT * FROM flow_users_homes WHERE houseID = @houseID")
vRP.prepare("flow/allAPOwner","SELECT * FROM flow_users_homes WHERE houseID = @houseID AND proprietario = @proprietario")
vRP.prepare("flow/criarGaragem","UPDATE flow_homes SET garagem = @garagem WHERE id = @houseID")
vRP.prepare("flow_homes/myVehicles","SELECT * FROM vrp_user_veiculos WHERE user_id = @user_id")
vRP.prepare("flow_homes/isMyVehicle","SELECT * FROM vrp_user_veiculos WHERE user_id = @user_id AND veiculo = @veiculo ")
vRP.prepare("flow_homes/updateArmario","UPDATE flow_users_homes SET armario = @armario WHERE id = @id")
vRP.prepare("flow/allInfoHome","SELECT * FROM flow_users_homes WHERE id = @id")
vRP.prepare("flow/updateBau","UPDATE flow_users_homes SET bau = @bau WHERE id = @id") 
vRP.prepare("flow/updateMorador","UPDATE flow_users_homes SET moradores = @moradores WHERE id = @id")
vRP.prepare("flow/selectProprietario","SELECT * FROM flow_users_homes WHERE proprietario = @proprietario")
vRP.prepare("flow/updateIptu","UPDATE flow_users_homes SET iptu = @iptu WHERE id = @id")
vRP.prepare("flow/updateOwner","UPDATE flow_users_homes SET proprietario = @proprietario WHERE id = @id")
vRP.prepare("flow/deleteUserProp","DELETE FROM flow_users_homes WHERE id = @id")
vRP.prepare("flow/deleteUsers","DELETE FROM flow_users_homes WHERE houseID = @houseID")
vRP._prepare("flow/getHousesIPTU", "SELECT id,houseID,proprietario FROM flow_users_homes WHERE iptu < UNIX_TIMESTAMP()")
vRP._prepare("flow/createDB",[[ CREATE TABLE IF NOT EXISTS `flow_homes` ( `id` int(11) NOT NULL AUTO_INCREMENT, `tipo` varchar(15000) NOT NULL, `price` int(11) NOT NULL, `coords` varchar(50) NOT NULL, `garagem` text DEFAULT '{}', `chaves` int(11) DEFAULT NULL, `minBau` int(11) NOT NULL, `permissao` varchar(50) NOT NULL, `interior` int(11) NOT NULL DEFAULT 1, `maxMoradores` int(11) DEFAULT NULL, PRIMARY KEY (`id`) ) ENGINE=InnoDB DEFAULT CHARSET=latin1; ]])
vRP._prepare("flow/createDB2",[[ CREATE TABLE IF NOT EXISTS `flow_users_homes` ( `id` int(11) NOT NULL AUTO_INCREMENT, `tipo` varchar(50) NOT NULL, `houseID` int(11) NOT NULL, `proprietario` int(11) NOT NULL, `moradores` text DEFAULT '{}', `bau` text DEFAULT '{}', `armario` text DEFAULT '{}', `interior` int(11) NOT NULL, `iptu` int(11) NOT NULL, PRIMARY KEY (`id`) ) ENGINE=InnoDB DEFAULT CHARSET=latin1; ]])
config.newVersion = true -- NAO MEXER

CreateThread(function()
    if config.createTable then
        vRP.execute("flow/createDB", {})
        vRP.execute("flow/createDB2", {})
    end
end)
