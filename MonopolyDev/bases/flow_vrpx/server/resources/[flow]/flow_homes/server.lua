...
criarApartamento = function(interior, tipo,price,chaves, minBau, coords, permissao)
    if config.oxmysql then
        local rows = exports["oxmysql"]:executeSync([[ INSERT INTO flow_homes(`interior`,`tipo`,`price`,`chaves`,`minBau`,`coords`,`permissao`) VALUES (?,?,?,?,?,?,?) ]], { interior,tipo,price,chaves,minBau,json.encode({x=coords[1],y=coords[2],z=coords[3]}),permissao })
        if rows then
        ...
        vRP.execute("flow/criarPropriedade", { interior = interior, tipo = tipo, price = price, chaves = chaves, minBau = minBau, coords = json.encode({x=coords[1],y=coords[2],z=coords[3]}), permissao = permissao })
        refreshHomes()
    end
end
...
criarCasa = function(interior, tipo,price,minBau, coords, permissao)
    if config.oxmysql then
        local rows = exports["oxmysql"]:executeSync([[ INSERT INTO flow_homes(`interior`,`tipo`,`price`,`chaves`,`minBau`,`coords`,`permissao`) VALUES (?,?,?,?,?,?,?) ]], { interior,tipo,price,1,minBau,json.encode({x=coords[1],y=coords[2],z=coords[3]}),permissao })
        if rows then
            local id = rows.insertId
            local newCoords = { x=coords[1], y=coords[2], z=coords[3] }
        ...
        vRP.execute("flow/criarPropriedade", { interior = interior, tipo = tipo, price = price, chaves = 1, minBau = minBau, coords = json.encode({x=coords[1],y=coords[2],z=coords[3]}), permissao = permissao })
        refreshHomes()
    end
end
...
criarGaragem = function(id, coords, spawnCoords)
    ...
    if id then
        ...
        vRP.execute("flow/criarGaragem", { houseID = id, garagem = json.encode(value) })
        ...
        if config.lotus then
            TriggerEvent("flow:getGarages", propriedades, id)
        end
    end
end
...
deletarPropriedade = function(id)
    local id = parseInt(id)
    if id then
        vRP.execute("flow/deletarPropriedade", { houseID = id })
        vRP.execute("flow/deleteUsers", { houseID = id })
...
comprarPropriedade = function(user_id, tipo, id, interior)
    ...
    if id then
        if config.lotus then
            if config.oxmysql then
                local rows = exports["oxmysql"]:executeSync([[INSERT INTO flow_users_homes (tipo,houseID,proprietario,interior,iptu) VALUES (?,?,?,?,?) ]], { tipo,id,user_id,interior,(os.time() + config.sellHouseIptu*24*60*60) })
                if rows then
                ...
                vRP.execute("flow/comprarPropriedade", { tipo = tipo, houseID = id, proprietario = user_id, interior = interior, iptu = (os.time() + config.sellHouseIptu*24*60*60) })
                refreshMoradores()
            end
        else
            if config.oxmysql then
                local rows = exports["oxmysql"]:executeSync([[INSERT INTO flow_users_homes (tipo,houseID,proprietario,interior,iptu) VALUES (?,?,?,?,?) ]], { tipo,id,user_id,interior,os.time() })
                if rows then
                    local rowsID = rows.insertId
                    if rowsID then
                        if proprietarios[user_id] == nil then
                ...
                vRP.execute("flow/comprarPropriedade", { tipo = tipo, houseID = id, proprietario = user_id, interior = interior, iptu = os.time() })
                refreshMoradores()
            end
        end
    end
...
refreshHomes = function()
    local rows = vRP.query("flow/allPropriedades", {})
    if rows then
        ...
        TriggerEvent("flow:getGarages", propriedades)
...
refreshMoradores = function()
    local rows2 = vRP.query("flow/allMoradores", {})
    if #rows2 > 0 then
...
src.checkIdHouse = function(id)
    local houseId = #vRP.query("flow/selecionarPropriedade", { houseID = id })
    if houseId > 0 then
        return true
    end
end
...
src.checkEnterAp = function(user_id, id, proprietario) -- return 1 = entra ap, return 2 = toca campanhia, return 3 nao existe esse numero
    local owner = vRP.query("flow/allAPOwner", { houseID = id, proprietario = proprietario })
    if #owner > 0 then 
...
src.getInfoHouseId = function(id, tipo, proprietario)
    if tipo == "casa" then
        local rows = vRP.query("flow/allHomeOwner", { houseID = id  })
        local house = rows[1]
    ...
    elseif tipo == "apartamento" then
        local rows = vRP.query("flow/allAPOwner", { houseID = id, proprietario = proprietario })
        local house = rows[1]
...
src.checkIsOwner = function(user_id, id)
    local owner = vRP.query("flow/allAPOwner", { houseID = id, proprietario = user_id })
    if #owner > 0 then
        return owner[1]
    end
end
...
src.getArmario = function(id, houseID)
    local rows = vRP.query("flow/allInfoHome", { id = id })
    if #rows > 0 then
...
src.salvarRoupas = function(id, name, custom)
    ...
    if #rows > 0 then
        roupa[name] = custom
        vRP.execute("flow_homes/updateArmario", { id = id, armario = json.encode(roupa) })
...
src.checkRemMorador = function(id, houseID, morador)
    ...
    if user_id then
        ...
        if #rows > 0 then
            ...
            vRP.execute("flow/updateMorador", { id = id, moradores = json.encode(moradores) } )
...
src.getMoradores = function(user_id, houseID)
   ...
   local owner = vRP.query("flow/allAPOwner", { houseID = houseID, proprietario = user_id })
    if #owner > 0 then 
        local id = owner[1].id
        local moradores = json.decode(owner[1].moradores)
                ...
                if v.iptu then
                    if v.iptu <= os.time() + config.iptuVencimento*24*60*60 then
                        ...
                        if vRP.tryFullPayment(user_id, price) then
                            ...
                            vRP._execute("flow/updateIptu", { iptu = (os.time() + config.sellHouseIptu*24*60*60) , id = v.id })
